<?php require 'php/header.php'; ?>

<div class="containeur">

  <div class="category-auto auto">

    <div class="sub-category">
      
      <div class="s-cat carrosserie" id='carrosserie'>
        <div class="s-cat-container"></div>
      </div>
      
        <div class="s-cat mecanique" id='mecanique'>
          <div class="s-cat-container"></div>
        </div>

    </div>
  </div>

<div class="category-auto astro">
  <div class="sub-category">

    <div class="s-cat astronomie" id='astronomie'>
      <div class="s-cat-container"></div>
    </div>

    <div class="s-cat cosmologie" id='cosmologie'>
      <div class="s-cat-container"></div>
    </div>

    <div class="s-cat observation" id='observation'>
      <div class="s-cat-container"></div>
    </div>

  </div>
</div>


<div class="category-auto culture">

  <div class="sub-category">
    <div class="s-cat livres" id='livres'>
      <div class="s-cat-container">
      </div>
    </div>
  
    <div class="s-cat religions" id='religions'>
    <div class="s-cat-container">
    </div>
  </div>


  </div>
</div>

  <div class="category-auto electronique">

    <div class="sub-category">
      <div class="s-cat" id='mecanique'>
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>

    </div>
    </div>



    <div class="category-auto gastronomie">

<div class="sub-category">
  <div class="s-cat" id='mecanique'>
    <div class="s-cat-container">
    </div>
  </div>

  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>

</div>
</div>



<div class="category-auto mode">

    <div class="sub-category">
      <div class="s-cat" id='mecanique'>
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>
      <div class="s-cat mecanique"></div>

    </div>
    </div>


    <div class="category-auto science">

<div class="sub-category">
  <div class="s-cat" id='mecanique'>
    <div class="s-cat-container">
    </div>
  </div>

  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>
  <div class="s-cat mecanique"></div>

</div>
</div>



<div class="category-auto sports">

    <div class="sub-category">
      <div class="s-cat aviron" id="aviron">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat canoekayak" id="canoekayak">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat kitesurf" id="kitesurf">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat natation" id="natation">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat plongee" id="plongee">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat paddle" id="paddle">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat regate" id="regate">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat surf" id="surf">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat voile" id="voile">
        <div class="s-cat-container">
        </div>
      </div>

      <div class="s-cat waterpolo" id="waterpolo">
        <div class="s-cat-container">
        </div>
      </div>

   </div>
</div>


<div class="category-auto voyages">

    <div class="sub-category">

      <div class="s-cat trouvemadestination" id='trouvemadestination'>
        <div class="s-cat-container"></div>
      </div>

      <div class="s-cat destinationprefere" id='destinationprefere'>
        <div class="s-cat-container"></div>
      </div>

  </div>
</div>



<div class="category-auto ans">

    <div class="sub-category">

      <div class="s-cat " id=''>
        <div class="s-cat-container"></div>
      </div>

      <div class="s-cat " id=''>
        <div class="s-cat-container"></div>
      </div>

    </div>
</div>

<div class="category-auto jeux">

    <div class="sub-category">

      <div class="s-cat arcade" id='arcade'>
        <div class="s-cat-container"></div>
      </div>

      <div class="s-cat mmo" id='mmo'>
        <div class="s-cat-container"></div>
      </div>

    </div>
</div>


<div class="category-auto informatique">

    <div class="sub-category">

      <div class="s-cat developpeur" id='developpeur'>
        <div class="s-cat-container"></div>
      </div>

      <div class="s-cat maintenance" id='maintenance'>
        <div class="s-cat-container"></div>
      </div>

      <div class="s-cat serveur" id='serveur'>
        <div class="s-cat-container"></div>
      </div>

    </div>
</div>




</div>

<?php require 'php/footer.php'; ?>