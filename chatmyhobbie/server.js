...
io.on('connection', function (socket) {
  ...
  socket.on('user-login', function (user, callback) {
    ...
    if (user !== undefined && userIndex === -1) { // S'il est bien nouveau
      ...
      var userServiceMessage = {
        text: 'L\'utilisateur "' + loggedUser.username + '" viens de se connecter',
        type: 'login'
      };
  ...
  socket.on('chat-message', function (message) {
    ...
    socket.broadcast.emit('chatr-messages', message);
...
/**
 * Lancement du serveur en écoutant les connexions arrivant sur le port 1300
 */
http.listen(1300, function () {
  console.log('Server is listening on *:1300');
});